//nvcc -ptx EM3_1.cu -ccbin "F:Visual Studio\VC\Tools\MSVC\14.12.25827\bin\Hostx64\x64"
__device__ void EM1( double * Er,
    ...
    Hphi[n_r] = Hphi[n_r] + (dt/(mu*dr)*( Ez[n_z+1]-Ez[n_z] ) - dt/(mu*dz)*( Er[n_r + threadsPerBlock]-Er[n_r] ));
