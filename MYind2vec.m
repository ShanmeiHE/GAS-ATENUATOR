...
%nargin=1, only one function input, which is a
%if nargin < 1,error(message('nnet:Args:NotEnough'));end

%wasMatrix = 1, which means a is a matrix
%wasMatrix = ~iscell(x);

% before x is a double matrix, after x becomes a cell.
x = nntype.data('format',x,'Argument');

% Nx,TS,M=1, Q is the length of the matrix x.
...
tic
b = sparse( a,gpuArray(1:length(a)), ones(length(a),1,'gpuArray') );
toc
